...
export function DocumentsPage() {
  ...
  const categories = ['all', 'identity', 'income', 'education', 'residence', 'category', 'other'] as const;
  const [activeCategory, setActiveCategory] = useState<typeof categories[number]>('all');

  const filtered = activeCategory === 'all' ? documents : documents.filter((d) => d.category === activeCategory);

  return (
    <DashboardLayout title="Document Vault" subtitle="Manage your documents securely in one place">

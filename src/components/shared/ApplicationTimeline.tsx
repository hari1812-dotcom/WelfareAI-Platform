        ...
        return (
          <div key={step.stage} className="flex gap-4">
            ...
            <div className={`pb-6 ${isLast ? 'pb-0' : ''}`}>
              <p className={`text-sm font-semibold ${step.status === 'pending' ? 'text-gray-400' : 'text-navy-800'}`}>{step.stage}</p>
              {step.date && <p className="text-xs text-gray-500 mt-0.5">{step.date}</p>}
              {step.status === 'current' && <span className="mt-1 inline-block text-xs font-semibold text-primary-600">Currently in progress</span>}
            </div>
          </div>
        );
      })}
